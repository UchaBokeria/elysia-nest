...
type InnerMacroToContext<A> = IsNever<A[keyof A]> extends false ? Exclude<Awaited<A[keyof A]>, AnyElysiaCustomStatusResponse | void> : {};
export type MacroToContext<MacroFn extends BaseMacroFn = {}, SelectedMacro extends MetadataBase['macro'] = {}> = {} extends SelectedMacro ? {} : InnerMacroToContext<{
    [key in keyof SelectedMacro as MacroFn[key] extends ResolveMacroFnLike ? key : MacroFn[key] extends ResolveMacroPropertyLike ? true extends SelectedMacro[key] ? key : never : never]: ResolveResolutions<ReturnTypeIfPossible<MacroFn[key]>['resolve']>;
}>;
...
export type ComposeElysiaResponse<Schema extends RouteSchema, Handle> = Handle extends (...a: any[]) => infer A ? _ComposeElysiaResponse<Schema, Replace<Awaited<A>, Blob, File>> : _ComposeElysiaResponse<Schema, Replace<Awaited<Handle>, Blob, File>>;
