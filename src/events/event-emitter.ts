import type { EventHandler, EventHandlerRegistration, IEventEmitter } from './event-emitter.interface';
...
export class EventEmitter implements IEventEmitter {
   ...
   */
  private readonly handlers: Map<string | symbol, EventHandlerRegistration[]> = new Map();
  ...
  off<T = any>(event: string | symbol, handler?: EventHandler<T>): void {
    ...
    if (handlers) {
      const filteredHandlers = handlers.filter(reg => reg.handler !== handler);
  ...
  async emit<T = any>(event: string | symbol, payload: T): Promise<void> {
    ...
    if (oneTimeHandlers.length > 0) {
      const remainingHandlers = handlers.filter(reg => !oneTimeHandlers.includes(reg));
