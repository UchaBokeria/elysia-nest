 ...
 */
export function OnEvent(event: string | symbol, options: OnEventOptions = {}): MethodDecorator {
  return (
    target: object,
    ...
    const events = Reflect.getMetadata(EVENTS_METADATA, target.constructor) || [];
...
export function registerEventHandlers(instance: any): void {
  ...
  const isSubscriber = Reflect.getMetadata(EVENT_LISTENER_METADATA, constructor);
