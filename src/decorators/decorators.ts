  ...
  return (metadataValue?: T): ParameterDecorator => {
    return (target, propertyKey, parameterIndex) => {
      ...
      if (propertyKey === undefined) {
        ...
        // Method parameter
        const existingParams = Reflect.getMetadata(metadataKey, target, propertyKey) || [];
        existingParams[parameterIndex] = value;
 ...
 */
export function getClassMetadata<T>(metadataKey: string, target: any): T | undefined {
...
export function getDecoratedProperties<T>(
  ...
  for (const propertyKey of Object.getOwnPropertyNames(target.prototype)) {
    if (Reflect.hasMetadata(metadataKey, target.prototype, propertyKey)) {
      properties.push({
        propertyKey,
        metadata: Reflect.getMetadata(metadataKey, target.prototype, propertyKey),
      });
    }
  }
  ...
  return (
    ...
    for (let i = decorators.length - 1; i >= 0; i--) {
      ...
      if (typeof propertyKey === 'undefined') {
        ...
        // Parameter decorator
        (decorator as ParameterDecorator)(target, propertyKey, descriptorOrIndex);
      } else {
        // Method decorator
