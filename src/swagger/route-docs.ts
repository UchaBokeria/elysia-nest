...
export function extractRouteDocs(controllers: any[]): OpenAPIPaths {
  ...
  for (const controllerClass of controllers) {
    ...
    const prefix = Reflect.getMetadata(ROUTE_PREFIX_METADATA, controllerClass) || '';
    ...
    const tag = controllerName.charAt(0).toLowerCase() + controllerName.slice(1);
    ...
    for (const route of routes) {
      ...
      const pathItem = generatePathItem(controllerClass, route, paramsMeta, tag, apiDocs);
...
function generatePathItem(
  ...
  if (apiDocs?.responses && pathItem.responses) {
    for (const [statusCode, responseData] of Object.entries(apiDocs.responses)) {
  ...
  for (const param of params) {
    switch (param.type) {
      ...
      case ParamType.QUERY:
        // Add query parameter
        if (param.paramName && pathItem.parameters) {
          const schema = param.dto ? getOrCreateSchema(param.dto) : { type: 'string' };
          pathItem.parameters.push({
            name: param.paramName,
            in: 'query',
        ...
        if (formDto) {
          ...
          if (schema) {
            ...
            requestBody = {
              required: true,
              description:
                apiDocs?.requestBodyDescription || apiDocs?.requestBody?.description || 'Form data',
              content: {
                [finalContentType]: {
                  schema: schema,
                  ...examples,
            ...
            if (schema.properties) {
              ...
              for (const key in schema.properties) {
                ...
                if (schema.properties[key]) {
                  ...
                  else if (apiDocs?.requestExample && apiDocs.requestExample[key] !== undefined) {
      ...
      case ParamType.FILE:
        ...
        requestBody.content['multipart/form-data'].schema.properties[fileFieldName] = {
          type: 'string',
          format: 'binary',
        ...
        if (apiDocs?.requestBodyDescription || apiDocs?.requestBody?.description) {
          ...
          if (schema && schema.properties) {
            ...
            for (const key in schema.properties) {
              if (schema.properties[key] && apiDocs.requestExample[key]) {
                ...
                if (requestBody.content['multipart/form-data'].schema.properties[key]) {
                  requestBody.content['multipart/form-data'].schema.properties[key].example =
                    apiDocs.requestExample[key];
                } else {
                  // Add the property with example if it doesn't exist yet
                  requestBody.content['multipart/form-data'].schema.properties[key] = {
      ...
      case ParamType.FILES:
        ...
        requestBody.content['multipart/form-data'].schema.properties[filesFieldName] = {
          type: 'array',
          items: {
            type: 'string',
            format: 'binary',
  ...
  if (requestDto && !hasBodyParam && !hasFormParam && !hasFileParam) {
    ...
    if (schema) {
      ...
      requestBody = {
        required: true,
        description: apiDocs?.requestBodyDescription || apiDocs?.requestBody?.description,
        content: {
          [contentType]: {
            schema: schema,
            ...examples,
