...
function resolveProvider(providerItem, moduleRef) {
  const moduleProviders = providers.get(moduleRef);
  if (!moduleProviders) {
    return null;
  }
...
function Module(options) {
  const moduleFactory = function() {
    const lifecycleManager2 = getLifecycleManager();
    const plugin = function(app) {
      const metadata = Reflect.getMetadata(MODULE_METADATA, moduleFactory);
      if (!metadata) {
      ...
      if (metadata.providers && metadata.providers.length > 0) {
        registerProviders(metadata.providers, moduleFactory);
        metadata.providers.forEach((provider) => {
          const instance = resolveProvider(provider, moduleFactory);
          if (instance) {
            providers3.push(instance);
            lifecycleManager2.register(instance);
          }
        });
      }
      providers3.forEach((provider) => {
        if (typeof provider === "object" && provider.onModuleInit) {
          provider.onModuleInit();
        }
      });
...
function Factory(ModuleClass) {
  const moduleInstance = new ModuleClass;
  return moduleInstance;
}
async function initializeApp(app, moduleClass) {
  const lifecycleManager2 = getLifecycleManager();
  app = Factory(moduleClass)(app);
  lifecycleManager2.triggerOnApplicationBootstrap();
  return app;
}

// src/core/http-methods.ts
...
var Put = methodDecorator("PUT");
var Patch = methodDecorator("PATCH");
var Delete = methodDecorator("DELETE");
